...
public class OtelEvent2SentryEventConverter {
  ...
  private io.sentry.event.Event doConvert(Event source) {
    ...
    for (ExceptionData exception : errorData.getExceptionsList()) {
      StackTraceInterface stackTraceInterface = convertStackTrace(exception.getStack());
      int pos = exception.getType().lastIndexOf('.');
      String exceptionClassName = exception.getType().substring(pos + 1);
      String exceptionPackageName = exception.getType().substring(0, pos);
      SentryException sentryException =
          new SentryException(exception.getMesssage(), exceptionClassName, exceptionPackageName,
              stackTraceInterface);
